...
const boolify = promise => promise.then(_ => true, _ => false)
...
const gitInit = async (
  ...
  const isInsideWorkTree = () =>
    boolify(shell('git rev-parse --is-inside-work-tree', {cwd}))
  const checkIgnore = () => boolify(shell('git check-ignore .', {cwd}))
...
const download = async generator => {
  ...
  await install([generator], {cwd: tempDir, silent: true})
...
const create = async argv => {
  ...
  if (pathType === 'local') {
    ...
    srcPath = await download(generator)
  }
  ...
  const go = {
    ...
    // polyfill native pipeline, see https://github.com/nodejs/node/blob/master/lib/internal/streams/pipeline.js#L1
    pipeline: promisify(stream.pipeline || pump),
    packages,
    modify,
    template,
  }
